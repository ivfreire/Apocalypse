...
void Map::Render(SDL_Renderer* rdr, Vector2 camera) {
	for (Layer layer : this->layers) {
		...
		new_rect.x = -camera.x;
		new_rect.y = -camera.y;
		SDL_RenderCopy(rdr, layer.texture, NULL, &new_rect);
	}
}
...
SDL_Surface* Map::GetTile(int id) {
	for (unsigned int i = 0; i < this->tilesets.size() - 1; i++) if (id > this->tilesets.at(i)->firstgid && id < this->tilesets.at(i+1)->firstgid) return this->tilesets.at(i)->GetTile(id);
	return this->tilesets.back()->GetTile(id);
}


void Map::LoadTilesets(tinyxml2::XMLElement* map) {
...
void Map::LoadLayers(tinyxml2::XMLElement* map) {
